...
processed_data = []  # Store processed data for combined plots
data_lock = Lock()   # Thread-safe data storage
...
def italicize_genus_name(genus_name):
    """Convert genus name to italic formatting, handling special cases"""
    if genus_name.startswith('Other ('):
        return genus_name  # Don't italicize 'Other' labels
    
    # Split by spaces and italicize only the first word (genus)
    parts = genus_name.split()
    if len(parts) > 0:
        parts[0] = f"${parts[0]}$"  # LaTeX-style italics for matplotlib
        return " ".join(parts)
    return genus_name
...
def process_single_file(args_tuple):
    ...
    # Store processed data for combined plots (thread-safe)
    processed_sample = {
    ...
    ax2.set_yticklabels(y_labels, fontsize=max(6, 9))  # Ensure minimum 6pt font
    ...
    for bar, count in zip(bars, df_major['count']):
        ax2.text(bar.get_width() + total_reads*0.01, bar.get_y() + bar.get_height()/2, 
                f'{count:,}', ha='left', va='center', fontsize=max(6, 9))
...
def create_combined_bar_plots(output_dir):
    """Create 3x2 combined bar plot layout"""
    global processed_data
    
    if not processed_data:
        print("No processed data available for combined plots")
        return
    
    # Sort by sample name for consistent ordering
    processed_data.sort(key=lambda x: x['sample_name'])
    
    # Take first 6 samples if more than 6 exist
    n_samples = min(6, len(processed_data))
    samples_to_plot = processed_data[:n_samples]
    
    # Calculate number of rows needed
    n_rows = math.ceil(n_samples / 2)
    n_cols = 2
    ...
    for i, sample_data in enumerate(samples_to_plot):
        ax = axes_flat[i]
        
        sample_name = sample_data['sample_name']
        df_major = sample_data['df_major']
        colors = sample_data['colors']
        total_reads = sample_data['total_reads']
        
        # Create bar plot
        y_pos = np.arange(len(df_major))
        bars = ax.barh(y_pos, df_major['count'], color=colors)
        ax.set_yticks(y_pos)
        ...
        ax.set_yticklabels(y_labels, fontsize=max(6, 8))  # Minimum 6pt font
        ax.set_xlabel('Number of Reads', fontsize=max(6, 9))
        ax.set_title(f'{sample_name}\n({total_reads:,} reads)', 
                    fontsize=max(6, 10), fontweight='bold')
        ax.grid(axis='x', alpha=0.3)
        
        # Add count labels
        for bar, count in zip(bars, df_major['count']):
            ax.text(bar.get_width() + total_reads*0.01, 
    ...
    # Save as SVG for Adobe Illustrator compatibility
    plt.savefig(f"{output_dir}/combined_16S_genus_bar_plots_95pct.svg", 
                bbox_inches='tight', dpi=300)
    plt.close()
    
    print(f"Combined bar plots saved to {output_dir}/combined_16S_genus_bar_plots_95pct.svg")
...
def main():
    ...
    parser.add_argument('--no-combined', action='store_true',
                       help='Skip creating combined 3x2 bar plot layout')
    ...
    # Create combined bar plots unless disabled
    if not args.no_combined:
        print("Creating combined 3x2 bar plot layout...")
        create_combined_bar_plots(args.output_dir)
