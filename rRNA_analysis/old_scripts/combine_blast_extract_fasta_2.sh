...
# Extract FASTA sequences for top hits with organism names as headers
echo "Extracting FASTA sequences for top $top_n hits with organism names..."
top_fasta="${output_dir}/top_${top_n}_sequences.fasta"

# Clear the output file
> "$top_fasta"

# Extract sequences one by one with custom headers
counter=0
while read subject_id; do
    counter=$((counter + 1))
    echo "Extracting sequence $counter/$top_n: $subject_id"
    ...
    # Get the sequence
    sequence=$(blastdbcmd -db "$blast_db" -entry "$subject_id" -outfmt "%s" 2>/dev/null)
    
    if [ -n "$sequence" ]; then
        # Create custom header with organism name, hit count, and original ID
        echo ">${organism} | hits:${hit_count} | ${subject_id}" >> "$top_fasta"
        echo "$sequence" >> "$top_fasta"
    else
        echo "WARNING: Could not extract sequence for $subject_id"
    fi
...
echo "Extracted $seq_count FASTA sequences with organism names"

if [ $seq_count -eq 0 ]; then
    echo "ERROR: No sequences extracted. Check database path and permissions."
fi
