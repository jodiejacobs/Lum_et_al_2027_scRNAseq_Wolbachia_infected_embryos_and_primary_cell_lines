...
def identify_doublets(adata, fig_dir):
    ...
    scrub = scr.Scrublet(adata.X, expected_doublet_rate=0.02) # requires an estimated rate as a prior 
    ...
    # scrub = scr.Scrublet(adata.X, expected_doublet_rate=expected_doublet_rate) # requires an estimated rate as a prior 
    # # Run the doublet detection
    # doublet_scores, predicted_doublets = scrub.scrub_doublets(min_counts=2, 
    #     min_cells=3, 
    #     min_gene_variability_pctl=85, 
    ...
    # # Try automatic threshold first
    # scrub.call_doublets()
    # detected_rate = scrub.predicted_doublets_.mean()

    # # Automatic valley-finding can fail on some samples, 
    # # landing on a spurious low threshold and flagging an
    # # implausibly large fraction of cells as doublets. Fall back to a
    # # quantile-based threshold tied to the expected doublet rate if so.
    # if detected_rate > 3 * expected_doublet_rate:
    #     print(f"Automatic threshold gave implausible doublet rate ({detected_rate:.1%}) — "
    #           f"falling back to quantile-based threshold")
    #     manual_threshold = np.quantile(scrub.doublet_scores_obs_, 1 - expected_doublet_rate)
    #     scrub.call_doublets(threshold=manual_threshold)
    #     print(f"Manual threshold set at doublet score = {manual_threshold:.4f}")
    # else:
    #     print(f"Automatic threshold accepted (detected rate = {detected_rate:.1%})")
