...
def analyze_cluster_relationships(adata, fig_dir, sample, cluster_col='leiden'):
    """
    Computes hierarchical clustering and plots a dendrogram.
    """
    print("\n" + "="*60)
    print(f"CALCULATING HIERARCHICAL CLUSTERING: {cluster_col}")
    print("="*60)

    # 1. Compute the dendrogram based on PCA representation
    # This links clusters by transcriptomic similarity
    sc.tl.dendrogram(adata, groupby=cluster_col)

    # 2. Save the standalone dendrogram
    sc.pl.dendrogram(adata, groupby=cluster_col, show=False)
    plt.savefig(os.path.join(fig_dir, f'{sample}_dendrogram.pdf'))
    plt.close()

def plot_ordered_dotplot(adata, genes, fig_dir, sample, cluster_col='leiden'):
    """
    Plots a DotPlot ordered by the dendrogram hierarchy.
    """
    # Using 'dendrogram=True' automatically reorders the Y-axis (clusters)
    sc.pl.dotplot(
        adata, 
        genes, 
        groupby=cluster_col, 
        ...
        standard_scale='var',
        show=False,
        save=f'_{sample}_ordered_titer_genes.pdf'
    )

def compare_cluster_phase_distributions(adata_wmel, adata_uninfected, fig_dir, sample, cluster_col='leiden'):
    """
    1. Plots phase distribution for BOTH Infected and Uninfected with identical dimensions.
    2. Performs Chi-square tests for each cluster.
    """
    print("\n" + "="*60)
    print("PLOTTING PHASE DISTRIBUTIONS (INFECTED VS UNINFECTED)")
    print("="*60)
    ...
    # 1. Plot Infected Distributions
    wmel_proportions = pd.crosstab(adata_wmel.obs[cluster_col], 
                                   adata_wmel.obs['phase'], 
                                   normalize='index') * 100
    
    plt.figure(figsize=fig_size)
    wmel_proportions.plot(kind='bar', stacked=True, color=phase_colors)
    plt.title(f"Infected (wMel): Phase Distribution per Cluster")
    plt.ylabel("Percentage of Cells")
    plt.legend(title="Phase", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, f'{sample}_infected_phase_distribution.pdf'))
    plt.close()

    # 2. Plot Uninfected Distributions (Identical Dimensions)
    uninf_proportions = pd.crosstab(adata_uninfected.obs[cluster_col], 
                                    adata_uninfected.obs['phase'], 
                                    normalize='index') * 100
    
    plt.figure(figsize=fig_size)
    uninf_proportions.plot(kind='bar', stacked=True, color=phase_colors)
    plt.title(f"Uninfected: Phase Distribution per Cluster")
    plt.ylabel("Percentage of Cells")
    plt.legend(title="Phase", bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, f'{sample}_uninfected_phase_distribution.pdf'))
    plt.close()

    # 3. Statistical Comparison per Cluster
    comparison_results = []
    common_clusters = set(adata_wmel.obs[cluster_col].unique()) & set(adata_uninfected.obs[cluster_col].unique())
    
    for cluster in sorted(list(common_clusters)):
        wmel_counts = adata_wmel.obs[adata_wmel.obs[cluster_col] == cluster]['phase'].value_counts()
        uninf_counts = adata_uninfected.obs[adata_uninfected.obs[cluster_col] == cluster]['phase'].value_counts()
        
        all_phases = ['G1', 'S', 'G2M']
        wmel_counts = [wmel_counts.get(p, 0) for p in all_phases]
        uninf_counts = [uninf_counts.get(p, 0) for p in all_phases]
        
        contingency = np.array([wmel_counts, uninf_counts])
        if np.any(contingency.sum(axis=0) == 0) or np.any(contingency.sum(axis=1) == 0):
            continue
            
        chi2, p_val, _, _ = chi2_contingency(contingency)
        comparison_results.append({'cluster': cluster, 'p_value': p_val})

    results_df = pd.DataFrame(comparison_results)
    results_df.to_csv(os.path.join(fig_dir, f'{sample}_cluster_phase_comparison_stats.csv'), index=False)
    return results_df

def plot_phase_shift_summary(adata_wmel, adata_uninfected, fig_dir, sample, cluster_col='leiden'):
    """
    Creates a heatmap summary of cell cycle shifts across clusters.
    """
    print("\n" + "="*60)
    print("PLOTTING PHASE SHIFT SUMMARY")
    print("="*60)

    # 1. Calculate Proportions
    wmel_prop = pd.crosstab(adata_wmel.obs[cluster_col], adata_wmel.obs['phase'], normalize='index')
    uninf_prop = pd.crosstab(adata_uninfected.obs[cluster_col], adata_uninfected.obs['phase'], normalize='index')

    # 2. Calculate the Difference (Infected - Uninfected)
    # Positive values = Increase in that phase when infected
    diff_prop = (wmel_prop - uninf_prop).fillna(0) * 100

    # 3. Plotting the Heatmap
    plt.figure(figsize=(12, 8))
    sns.heatmap(diff_prop, annot=True, cmap='RdBu_r', center=0, fmt=".1f")
    plt.title(f"Cell Cycle Phase Shift (% Change: Infected - Uninfected)\n{sample}")
    plt.xlabel("Cell Cycle Phase")
    plt.ylabel("Leiden Cluster")
    
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, f'{sample}_phase_shift_heatmap.pdf'))
    plt.close()

    print(f"Phase shift heatmap saved to {fig_dir}")
...
def main():
    ...
    # --- FIX 1: Create output dir and tell Scanpy to use it for figures ---
    ...
    if adata_wmel is None: return

    # Detect titer column
    titer_col = args.titer_column or detect_titer_column(adata_wmel)
    
    # Scoring
    if args.run_scoring or 'phase' not in adata_wmel.obs.columns:
        adata_wmel = score_cell_cycle_scanpy(adata_wmel, args.output, 'wMel-infected')
    
    if adata_uninfected is not None:
        if args.run_scoring or 'phase' not in adata_uninfected.obs.columns:
            adata_uninfected = score_cell_cycle_scanpy(adata_uninfected, args.output, 'Uninfected')

    # Run analyses
    titer_results = analyze_titer_cellcycle_association(adata_wmel, args.output, 'wMel-infected', titer_col)
    
    comparison_results = None
    if adata_uninfected is not None:
        comparison_results = compare_cellcycle_distributions(adata_wmel, adata_uninfected, args.output, 'JW18_comparison')
    
    cluster_results = analyze_titer_by_cluster(adata_wmel, args.output, 'wMel-infected', titer_col)
    ...
    if cluster_results and cluster_results['p_kw'] < 0.05:
        ...
        sc.pl.dotplot(adata_wmel, top_genes, groupby='leiden', 
                      standard_scale='var', 
                      save='_titer_associated_top_genes.pdf', 
                      show=False)

    if cluster_results:
        # Compute the hierarchy first
        analyze_cluster_relationships(adata_wmel, args.output, 'wMel-infected')

        if cluster_results['p_kw'] < 0.05:
            de_genes = find_titer_associated_genes(adata_wmel, cluster_results)
            top_genes = de_genes['names'].head(10).tolist()
            
            # Plot the dotplot using the dendrogram order
            plot_ordered_dotplot(adata_wmel, top_genes, args.output, 'wMel-infected')

    analyze_titer_phase_intersection(adata_wmel, args.output, 'JW18wMel-Ctrl')

    if adata_uninfected is not None:
        # 1. Statistical Comparison
        comparison_stats = compare_cluster_phase_distributions(adata_wmel, adata_uninfected, args.output, 'JW18_comparison')
        
        # 2. Visual Summary Heatmap
        plot_phase_shift_summary(adata_wmel, adata_uninfected, args.output, 'JW18_comparison')

    # Summary
    print("\n" + "="*60 + "\nANALYSIS SUMMARY\n" + "="*60)
    if cluster_results:
        # --- FIX 3: Correct key name from p_value to p_kw ---
        print(f"\nTiter-Cluster association:")
        print(f"  Kruskal-Wallis p = {cluster_results['p_kw']:.2e}")
        top_cluster = cluster_results['stats'].index[0]
        print(f"  Highest titer cluster: {top_cluster}")
